...
const teamEdit = useEditTeamPayload()
...
const onDeleteRow = () => {
  ...
  if (teamEdit.employees) {
    teamEdit.employees = teamEdit.employees.filter((_, i) => i !== props.index)
  }
}
...
<template>
  <div class="font-secondary add-member-row gap-x-4 gap-y-4">
    ...
    <n-input-group class="code">
      <n-input-group-label class="bg-panel-secondary">{{code || team.code || teamEdit.code || '-'}}</n-input-group-label>
      <n-input 
